...
instance ToShape d => Floating (Expression d R) where
  ...
  -- | Overload exponential, log functions and pi
...
instance ToShape d => Num (Expression d Covector) where
  ...
  -- | Overload operators and define common transformations
  e1 + e2 = -- | Sum two covector expressions iff they have the same dimension
    let op = naryET Sum ElementDefault `hasShape` expressionShape e1
     in ensureSameShape e1 e2 $ applyBinary op e1 e2
  (*) = error "Not applicable to 1-form"
  negate = -- | Negation of covector expression
    let op = unaryET Neg ElementDefault
     in applyUnary $ unaryET Neg ElementDefault
  fromInteger = error "Not applicable to 1-form"
  abs = error "TODO: abs" -- | Absolute value of covector
  signum = error "Not applicable to 1-form"

-- TODO Scale in vector space comments
...
instance (DimensionType d) => ComplexRealOp (Expression d R) (Expression d C) where  
  ...
  (+:) e1 e2 = -- Make complex expression with e1 as real part, and e2 and imaginary part
    let op = binary RealImag
     in ensureSameShape e1 e2 $ applyBinary op e1 e2
  xRe :: Expression d C -> Expression d R
  xRe = -- Get real part of complex expression, returned as its own expression
    let op = unary RealPart
     in applyUnary op
  xIm :: Expression d C -> Expression d R
  xIm = -- Get imaginary part of complex expression, returned as its own expression
    let op = unary ImagPart
     in applyUnary op

-- TODO inner product comments
instance
...
huber delta e = piecewise [- delta, delta] e [outerLeft, inner, outerRight]
  --    @
  --    let z = variable1D @5 "var"
  --    huber 3 z
  --    @
  where
    one = constWithShape @d (expressionShape e) 1
    inner = constant 0.5 *. (e * e)  -- 1/2*e^2, for |e| <= δ
    -- δ(|e| - 1/2*δ) 
...
-- TODO Isn't the Euclidian norm in R^n: sqrt (sumElements(expr * expr))?
-- | Euclidian norm
...
instance (DimensionType d, ElementType et) => PiecewiseOp (Expression d R) (Expression d et) where
  -- TODO I don't know how this works yet or how to comment it
...
instance (DimensionType d) => FTOp (Expression d R) (Expression d C) where  
  ...
  --    @
  --    let exp = (fromDouble 15) :: Expression Scalar R
  --    ft exp
  --    @  
...
-- | Create a variable for one-dimensional nat values
variable1D ::
  forall n.
  (KnownNat n) =>
  String -> 
...
variable2D name = Expression h (fromList [(h, node)])
  -- | Restrict size with @m @n 
  --     @
  --     let exp = variable2D "var"
  --     let exp = variable2D @10 @20 "var"
  --     @
  where
...
variable3D name = Expression h (fromList [(h, node)])
  -- | Restrict size with @m @n @p
  --     @
  --     let exp = variable3D "var"
  --     let exp = variable3D @10 @20 @30 "var"
  --     @
  where
    size1 = valueFromNat @m
    size2 = valueFromNat @n
...
-- | Constant to expression, without KnownNat constraint
...
-- TODO Explanation as to reason for satifying KnownNat constraint
